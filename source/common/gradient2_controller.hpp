...
class Gradient2Controller
{
    public:
        ...
        Gradient sample(uint64_t now_ns, uint64_t rtt, uint32_t limit, uint32_t inflight)
        {
            ...
            if (now_ns > next_update_time_.load())
            {
                ...
                if (now_ns > next_update_time_.load())
                {
                    ...
                    next_update_time_.exchange(now_ns + ((uint64_t)sample_window_time_ms * 1000000));
                    ...
                    // TODO why shortRTT is so high? (bug?)
                    // TODO report recalculations

                    gradient_ = (struct Gradient){.limit = limit, .shortRtt = shortRtt, .longRtt = (uint32_t)longRtt, .inflight = maxInflight};
                }
            }
